"""Lyrics generation wrapper model for the Transformer backend."""
...
class Lyrics(torch.nn.Module):
    """Wrapper class for the Transformer model."""
    def __init__(self, config=Hyperparameters()):
        """
        Initialize the Lyrics class.

        Parameters
        ----------
        config : Hyperparameters, optional
            Configuration parameters for the Lyrics class. Default is Hyperparameters().
        """
        super().__init__()
    ...
    def _tokenizer(self):
        ...
        tokenizer.train(
            text=random.sample(text, k=k) if k else " ".join(text),
    ...
    def forward(self, text, margin=1500, temperature=1.0, top_k=None):
        ...
        temperature : float, optional
            The temperature for the softmax function. Default is 1.0.
        top_k : int, optional
            The number of top-k tokens to consider. Default is None.
