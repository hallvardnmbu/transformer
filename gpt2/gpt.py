"""GPT-2 text generation wrapper model for the Transformer backend."""
...
class GPT(torch.nn.Module):
    ...
    def __init__(self, config=Hyperparameters()):
        """
        Initialize the GPT-2 class.

        Parameters
        ----------
        config : Hyperparameters, optional
            Configuration parameters for the GPT-2 class. Default is Hyperparameters().
        """
        super().__init__()
    ...
    def get_batch(self, train=True):
        ...
        ----------
        train : bool, optional
            Whether to sample from the train or test set.

        Returns
        -------
        torch.Tensor, torch.Tensor
        """
        data = self.data["train"] if train else self.data["test"]
        ix = torch.randint(len(data) - self.config.block_size,
                           (self.config.batch_size,))

        x = torch.stack([torch.tensor(data[i:i + self.config.block_size]) for i in ix])
        y = torch.stack([torch.tensor(data[i + 1:i + 1 + self.config.block_size]) for i in ix])
    ...
    def learn(self, checkpoints=True, sentence=None):
        ...
        checkpoint = self.config.epochs // 25 if checkpoints else self.config.epochs
    ...
    def evaluate(self):
        ...
        for k in range(self.config.eval_iters):
            x, y = self.get_batch(train=False)
            with self.ctx:
                _, loss = self.transformer(x, y)
            losses[k] = loss.item()
