...
class Hyperparameters:
    """
    A class used to represent the hyperparameters of the GPT-2 model.

    Attributes
    ----------
    gpt2 : bool, optional
        Wheter to use the pretrained backend of GPT-2.
    block_size : int, optional
        Size of the blocks. Default is 256.
    micro_steps : int, optional
        Number of micro steps. Default is 8 * 4.
    vocab_size : int, optional
        Size of the vocabulary. Default is 24540.
    n_layer : int, optional
        Number of layers. Default is 6.
    n_head : int, optional
        Number of attention heads. Default is 6.
    n_embd : int, optional
        Dimensionality of embeddings and hidden states. Default is 384.
    ...
    epochs : int, optional
        Number of epochs for training. Default is 5000.
    ...
    eval_iters : int, optional
        The number of iterations for evaluation. Default is 2.
    ...
    Notes
    -----
    If `gpt2 = True` then the following parameters must be set to these values:
    - `block_size = 1024`
    - `vocab_size = 50257`
    - `n_layer = 12`
    - `n_head = 12`
    ...
    optimizer: dict[str, int or float] = field(
        default_factory=lambda: {
            "lr": 1e-5, "betas": (0.9, 0.99), "eps": 1e-9,
        }
    )
    scheduler: dict[str, int] = field(
        default_factory=lambda: {
            "decay_lr": False,
            "warmup": None, "max": None, "min_lr": None,
        }
    )
    ...
    tokenizer: dict[str, int or dict[str, int]] = field(
        default_factory=lambda: {
            "path": "gpt2",  # None: train new tokenizer else path to huggingface tokenizer
